...
export default function AdminApp({ user }: { user: AdminUser }) {
  ...
  async function handleLogout() {
    try {
      ...
      // session cookie is cleared client-side regardless of response
    }
      ...
      <div className="grid min-h-[calc(100vh-2rem)] gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="sticky top-4 flex h-fit flex-col gap-6 rounded-md border border-slate-800 bg-slate-900 p-5">
          ...
          <button
            ...
            className="rounded-md border border-slate-800 px-5 py-3 text-center text-sm font-semibold text-slate-400 transition hover:border-slate-600 hover:bg-slate-800 hover:text-slate-100"
          >
            Sign out
          </button>
        </aside>
